...
#define torqueRegKp 0.80f
#define torqueRegKi 0.05f
#define torqueRegKd 0.00f
#define torqueRegIntegralMin -500.0f
#define torqueRegIntegralMax 500.0f
...
static float PidDelta(float error, float dtSeconds, float &integral, float &previousError) {
    integral += error * dtSeconds;
    integral = ClampFloat(integral, torqueRegIntegralMin, torqueRegIntegralMax);
    float derivative = 0.0f;
    if (dtSeconds > 0.0f) {
        derivative = (error - previousError) / dtSeconds;
    }
    previousError = error;
    return (torqueRegKp * error) + (torqueRegKi * integral) + (torqueRegKd * derivative);
}
...
int main(void) {
    ...
    float buttonFullmovPidIntegral = 0.0f;
    float buttonHalfmovPidIntegral = 0.0f;
    float buttonFullmovPrevError = 0.0f;
    float buttonHalfmovPrevError = 0.0f;
    ...
    while (true) {
        ...
        if (motorEnabled && manualJogMode) {
            ...
            if (buttonRisingEdge && homing.state == HOMING_IDLE &&
                ...
                buttonFullmovPidIntegral = 0.0f;
                buttonFullmovPrevError = 0.0f;
            ...
            if (buttonHalfRisingEdge && homing.state == HOMING_IDLE &&
                ...
                buttonHalfmovPidIntegral = 0.0f;
                buttonHalfmovPrevError = 0.0f;
            ...
            if (!manualJogMode &&
                ...
                if (torqueRegIntervalMs == 0 ||
                    ...
                    float dtSeconds = static_cast<float>(nowMs - lastTorqueRegMs) / 1000.0f;
                    if (dtSeconds <= 0.0f) {
                        dtSeconds = 0.001f;
                    }
                    ...
                    if (useA11HzControl) {
                        if (proxAvgReady) {
                            ...
                            if (buttonFullmovState == BUTTONFULLMOV_RUNNING) {
                                buttonFullmovRpmCommand += PidDelta(
                                    error, dtSeconds,
                                    buttonFullmovPidIntegral,
                                    buttonFullmovPrevError);
                                buttonFullmovRpmCommand = ClampFloat(
                                ...
                                buttonHalfmovRpmCommand += PidDelta(
                                    error, dtSeconds,
                                    buttonHalfmovPidIntegral,
                                    buttonHalfmovPrevError);
                                buttonHalfmovRpmCommand = ClampFloat(
