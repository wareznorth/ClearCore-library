...
int main(void) {
    ...
    uint32_t proxAvgStartMs = Milliseconds();
    uint32_t proxAvgSamples = 0;
    float proxAvgSum = 0.0f;
    ...
    while (true) {
        ...
        if (proxWindowUpdated || proxTimedOut || ConnectorA12.State()) {
            ...
            proxAvgSum += proxHz;
            proxAvgSamples++;
            if ((Milliseconds() - proxAvgStartMs) >= proxAvgWindowMs &&
                proxAvgSamples > 0) {
                proxHzAvg = proxAvgSum / static_cast<float>(proxAvgSamples);
                proxAvgSum = 0.0f;
                proxAvgSamples = 0;
                proxAvgStartMs = Milliseconds();
                proxAvgReady = true;
            }
        }
