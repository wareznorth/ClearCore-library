    ...
    while (true) {
        ...
        if (motorEnabled && manualJogMode) {
            ...
            if (buttonRisingEdge && homing.state == HOMING_IDLE &&
                ...
                proxZeroWindows = 0;
                proxLastUpdateMs = Milliseconds();
                buttonMoveStartMs = Milliseconds();
                hlfbDutySum = 0.0f;
                hlfbDutyCount = 0;
                hlfbAvgReady = false;
            ...
            if (stallState == STALL_IDLE &&
                (buttonFullmovState == BUTTONFULLMOV_RUNNING ||
                 buttonHalfmovState == BUTTONHALFMOV_RUNNING) &&
                (Milliseconds() - buttonMoveStartMs) >=
                    (proxWindowMs * proxZeroWindowsForStall) &&
                ...
                if (torqueRegIntervalMs == 0 ||
                    ...
                    if (useA11HzControl) {
                        ...
                        bool startupDelayElapsed =
                            (Milliseconds() - buttonMoveStartMs) >= torqueControlStartDelayMs;
                        if (hlfbAvgReady && startupDelayElapsed) {
                            float dutyForControl = hlfbDutyAvg;
